...
def live_run(c, port=8080):
    ...
    server = Server()
    server.watch('app/')
    server.watch('app/data/*.json', build_site)
    server.watch('templates/*.html', build_site)
...
def publish_github(c):
    ...
    with c.prefix("source .venv/bin/activate"):
        c.run('ghp-import -r origin -b master -pm "(updated): site updated" app', replace_env=False)
